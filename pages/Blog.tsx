    ...
    <div className="max-w-4xl mx-auto pb-12">
      ...
      {selectedBlog ? (
        <div className="bg-white/95 backdrop-blur rounded-3xl shadow-xl border border-white overflow-hidden animate-fade-in">
          ...
          <div className="p-10">
            ...
            <div className="flex items-center justify-between">
              <button onClick={() => likeBlog(selectedBlog.id)} className="flex items-center gap-2 text-rose-500 font-bold hover:bg-rose-50 px-4 py-2 rounded-full transition-colors"><Heart size={20} className={selectedBlog.likes > 0 ? "fill-current" : ""} /> {selectedBlog.likes} 点赞</button>
              <div className="flex items-center gap-2 text-slate-500 font-medium"><MessageSquare size={20} /> {selectedBlog.comments.length} 评论</div>
            </div>
          </div>
        </div>
      ) : (
        ...
        <div className="space-y-8">
          {blogs.map((blog) => (
            <div key={blog.id} onClick={() => setSelectedBlog(blog)} className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 shadow-sm border border-white hover:shadow-xl hover:shadow-amber-100 transition-all cursor-pointer group flex flex-col md:flex-row gap-6 relative">
              <div className="w-full md:w-56 h-56 shrink-0 rounded-xl overflow-hidden shadow-inner">
              ...
              <div className="flex-1 flex flex-col py-2">
                <div className="flex items-center gap-3 text-xs text-slate-400 mb-3 font-medium">
                  <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">{blog.tags[0]}</span>
                  <span>{new Date(blog.date).toLocaleDateString()}</span>
                </div>
                <h2 className="text-2xl font-bold text-slate-800 mb-3 group-hover:text-amber-600 transition-colors">{blog.title}</h2>
                <p className="text-slate-600 text-base mb-4 line-clamp-2 leading-relaxed">{blog.excerpt}</p>
                <div className="mt-auto flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <img src={blog.author.avatar} alt={blog.author.name} className="w-8 h-8 rounded-full border border-slate-200" />
                    <span className="text-sm text-slate-600 font-bold">{blog.author.name}</span>
                  </div>
                  <div className="flex items-center gap-4 text-slate-400 text-sm font-medium">
                    <span className="flex items-center gap-1 group-hover:text-rose-500 transition-colors"><Heart size={16} /> {blog.likes}</span>
